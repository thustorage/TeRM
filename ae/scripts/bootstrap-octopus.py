...
class RunPrinter:
    def __init__(self, name, channel):
        ...
        self.name = name
        self.dying = False
        self.server_inited = False
        
    def print_one(self):
        if self.channel.recv_ready():
            res = self.channel.recv(1048576 * 10).decode().splitlines()
            for l in res:
                print("@%-10s" % self.name, l.strip())
                if "Worker" in l:
                    self.server_inited = True

        if self.channel.recv_stderr_ready():
            res = self.channel.recv_stderr(1048576 * 10).decode().splitlines()
            for l in res:
                print("@%-10s" % self.name, l.strip())
...
class ConnectProxy:
    ...
    def connect(self, pwd, passp=None, timeout=30):
        ...
        # if user_config:
        #     print("found user_config", config)
        #     print(user_config)
        #     #print(user_config)
    ...
    def connect_with_pkey(self, keyfile_name, timeout=10):
        ...
        self.ssh.connect(hostname=self.mac, username=self.user,
                         key_filename=keyfile_name, timeout=timeout)
            ...
            if os.path.isfile(os.path.join(source, item)):
                try:
                    self.sftp.put(os.path.join(source, item),
                                  '%s/%s' % (target, item))
                    print_v(verbose, "[%s] put %s done" %
                            (self.mac, os.path.join(source, item)))
                except:
                    print("[%s] put %s error" %
                          (self.mac, os.path.join(source, item)))
            else:
                self.mkdir('%s/%s' % (target, item), ignore_existing=True)
                self.copy_dir(os.path.join(source, item),
                              '%s/%s' % (target, item))
...
class Courier2:
    ...
    def pre_execute(self, cmd, host, pty=True, dir="", timeout=None, retry_count=3, background=False):
        ...
        try:
            ccmd = ("cd %s" % dir) + ";" + str(self.envs) + cmd
            if not background:
                _, stdout, _ = p.execute(
                    ccmd, pty, timeout, background=background)
                return p, stdout
            else:
    ...
    def execute(self, cmd, host, pty=True, dir="", timeout=None, output=True, background=False):
        ret = [True, ""]
        p, stdout = self.pre_execute(
            cmd, host, pty, dir, timeout, background=background)
...
def main():
    ...
    arg_parser.add_argument('-b', '--black', nargs='+',
                            help='hosts to ignore', required=False)
    arg_parser.add_argument(
        '-n', '--num', help='num-passes to run', default=128, type=int)
    ...
    pwd = config.get("pwd", "fast24ae")
    passp = config.get("passphrase", None)

    prefix_cmd = config.get("prefix_cmd", "")
    print(f"! prefix_cmd={prefix_cmd}")
    suffix_cmd = config.get("suffix_cmd", "")
    print(f"! suffix_cmd={suffix_cmd}")
    ...
    for p in passes:
        ...
        if p.get("local", "no") == "yes":
            ...
            channel = cr.execute_w_channel(prefix_cmd + " " + p["cmd"] + " " + suffix_cmd,
                                           p["host"],
                                           p["path"])
            if p["name"] not in config.get("null", []):
                current_printer = RunPrinter(p["host"] + ":" + p["name"], channel)
                printer_list.append(current_printer)
                if p["name"] == "s0":
                    goon = True
                    while goon and not current_printer.server_inited:
                        goon = current_printer.print_one()
                        time.sleep(0.1)
    ...
    while printer_go_on:
        try:
            ...
            for p in printer_list:
                if should_stop:
                    p.dying = True
                    p.channel.send(chr(3))
                if p.print_one():
                    temp.append(p)
                else:
                    should_stop = True
            printer_list = temp
            printer_go_on = True if len(printer_list) > 0 else False
            time.sleep(0.1)
        except KeyboardInterrupt:
            print("set should_stop")
            should_stop = True
