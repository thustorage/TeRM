...
class RunPrinter:
    ...
    def print_one(self):
        if self.channel.recv_ready():
            res = self.channel.recv(1048576).decode().splitlines()
            for l in res:
                print("@%-10s" % self.name, l.strip())

        if self.channel.recv_stderr_ready():
            res = self.channel.recv_stderr(1048576).decode().splitlines()
            for l in res:
                print("@%-10s" % self.name, l.strip())
    ...
    for p in passes:
        ...
        if p.get("local", "no") == "yes":
            ...
            if p["name"] not in config.get("null", []):
                printer.append(RunPrinter(p["host"] + ":" + p["name"], channel))
            time.sleep(1)
        ...
        try:
            ...
            for p in printer:
                ...
                if p.print_one():
                    ...
                    should_stop = True
            printer = temp
            printer_go_on = True if len(printer) > 0 else False
            time.sleep(0.1)
